...
function income_graph_tbl()
{
    ...
    for ($i = $s_from_date; $i <= $s_to_date; $i->modify('+1 day')) {
        ...
        if (mysqli_num_rows($query)) {
            while ($row = mysqli_fetch_array($query)) {
                ...
                $income = $credit - $debit;
                $admin_charge += $row['transaction_charge'];
            }
        }else{
            $income = 0;
        }
    ?>
        <tr>
            ...
            <td><?php echo $c_symbol . $admin_charge; ?></td>
...
function user_income_summary($user_id)
{
    global $conn;
    global $income_tbl;
    global $c_symbol;
    global $base_url;
    $query = mysqli_query($conn, "SELECT * FROM $income_tbl WHERE user_id = '$user_id' ORDER BY date DESC");
    $count = 0;
    while ($row = mysqli_fetch_array($query)) {
        $count += 1;
        $user_name = user_name($user_id);
...
function referral_income_tbl($user_id)
{
    ...
    global $c_symbol;
    global $base_url;
    $query = mysqli_query($conn, "SELECT * FROM $income_tbl WHERE user_id = '$user_id' AND category = '2'  ORDER BY date DESC ");
    $count = 0;
    while ($row = mysqli_fetch_array($query)) {
        $count += 1;
        $user_name = user_name($user_id);
