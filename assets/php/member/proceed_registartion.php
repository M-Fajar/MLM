...
if (!isset($_POST['referral_id'], $_POST['placement_id'], $_POST['referral_code'], $_POST['first_name'], $_POST['last_name'], $_POST['user_name'], $_POST['mobile_number'], $_POST['email'], $_POST['password'], $_POST['confirm_password'], $_POST['placement_type'])) {
...
$get_generation_level = mysqli_fetch_array(mysqli_query($conn, "SELECT `generation_level` FROM $tree_tbl WHERE `referral_id`='$referral_id' AND `placement_type` = 'left' ORDER BY `id` DESC LIMIT 1"));
if(isset($get_generation_level)){
    if($get_generation_level['generation_level']+1 > 3){
        $generation_level = 1;
    }
    else{
        $generation_level = $get_generation_level['generation_level'] + 1;
    }
}else{
    $generation_level = 1;
}
$tree_query = mysqli_query($conn, "INSERT INTO $tree_tbl ( `user_id`, `referral_id`, `placement_id`, `placement_type`, `left_count`, `right_count`, `left_id`, `right_id`, `pair_count`,`generation_level`)
                 VALUES ('$user_id','$referral_id','$placement_id','$placement_type','$left_count','$right_count','$left_id','$right_id', '$pair_count', $generation_level)");
if (!$tree_query) {
...
binary_count($referral_id, $placement_type);
