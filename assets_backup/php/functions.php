...
function check_capping_update_balance($user_id, $income, $income_category, $agent_id){
    global $conn;
    global $balance_tbl;
    global $current_date;
    global $income_tbl;
    ...
    // Income Category 1 = Pair Income , 2 = Referral Income, 3 = Reward Income
    ...
    if (!mysqli_num_rows($query)) {
        ...
        mysqli_query($conn, "INSERT INTO $capping_tbl (`user_id`, `income`, `date`) 
            VALUES ('$user_id','$income','$date')  ");
    } else {
        $data = mysqli_fetch_array($query);
        ...
        mysqli_query($conn, "UPDATE $balance_tbl SET wallet = wallet+$income, income = income+$income , total_income = total_income+$income WHERE user_id = '$user_id'  ");
        mysqli_query($conn, "INSERT INTO $income_tbl ( `user_id`, `amount`, `date`, `agent_id`, `description`, `category` ) 
    VALUES('$user_id','$amount','$current_date','$agent_id','$pair_description','$income_category') ");
        mysqli_query($conn, "INSERT INTO $transaction_tbl (`user_id`, `amount`, `transaction_charge`, `net_amount`, `description`,`category`, `date`, `status`)
